...
public class RedisClusterScan {
    ...
    public Object deleteKeysByRegex(String regex) {
        ScanPage scan = new ScanPage();
        scan.setQuery(regex);
        do {
            HostAndPort hp = nodes[scan.getClient()];
            Jedis jedis = new Jedis(hp.getHost(), hp.getPort(), 3000);
            try {
                ScanResult<String> result = jedis.scan(scan.getCursor(), new ScanParams().count(1000).match(scan.getQuery()));
                scan.setCursor(result.getStringCursor());
                List<String> keys = result.getResult();
                if (0 != keys.size()) {
                    jedis.del(result.getResult().toArray(new String[]{}));
                }
                //当前结点没有数据了
                if (scan.getCursor() == null || "".equals(scan.getCursor()) || "0".equals(scan.getCursor())) {
                    scan.setClient(scan.getClient() + 1);
        ...
        } while (!"0".equals(scan.getCursor()) && scan.getClient() <= nodes.length && scan.getHasMore());
        return null;
    }
}